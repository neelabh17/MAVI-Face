...
def saveImg(imgName,myimg_raw,dets):
    if args.save_image:
        for b in dets:
            ...
            text = "{:.4f}".format(b[4])
            b = list(map(int, b))
            cv2.rectangle(myimg_raw, (b[0], b[1]), (b[2], b[3]), (0, 0, 255), 2)
            cx = b[0]
            cy = b[1] + 12
            cv2.putText(myimg_raw, text, (cx, cy),
                        cv2.FONT_HERSHEY_DUPLEX, 0.5, (255, 255, 255))

            # landms
            cv2.circle(myimg_raw, (b[5], b[6]), 1, (0, 0, 255), 4)
            cv2.circle(myimg_raw, (b[7], b[8]), 1, (0, 255, 255), 4)
            cv2.circle(myimg_raw, (b[9], b[10]), 1, (255, 0, 255), 4)
            cv2.circle(myimg_raw, (b[11], b[12]), 1, (0, 255, 0), 4)
            cv2.circle(myimg_raw, (b[13], b[14]), 1, (255, 0, 0), 4)
        # save image

        name = imgName
        # anti transposing
        cv2.imwrite(name, myimg_raw)
...
if __name__ == '__main__':
    ...
    for i in range(1):
        ...
        # show image
        saveImg("a4.jpg",img_raw.copy(),dets)
