...
for name,params in net.named_parameters():
    ...
    net.LandmarkHead = net._make_landmark_head(fpn_num=3, inchannels=cfg['out_channel'])
    ...
    for iteration in range(start_iter, max_iter):
        if iteration % epoch_size == 0:
            ...
            if (epoch % 1 == 0 and epoch > 0) or (epoch % 5 == 0 and epoch > cfg['decay1']):
                torch.save(net.state_dict(), save_folder + cfg['name']+ '_epoch_' + str(epoch) + '_noGrad_FT.pth')
            epoch += 1
        ...
        if iteration % epoch_size == 0:
            train_eval(net,dataset_,batch_size,epoch)
    
    torch.save(net.state_dict(), save_folder + cfg['name'] + '_Finally_FT.pth')
...
def train_eval(model,val_data,batch_size_val,epoch_no):
    print('Evaluating validation result for epoch => ',epoch_no)
    loss_val = 0.0
