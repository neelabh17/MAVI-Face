...
optimizer = optim.SGD(Plist, lr=initial_lr, momentum=momentum, weight_decay=weight_decay)
...
def train():
    ...
    for iteration in range(start_iter, max_iter):
        if iteration % epoch_size == 0:
            ...
            if (epoch % 2 == 0 and epoch > 0) or (epoch % 5 == 0 and epoch > cfg['decay1']):
                torch.save(net.state_dict(), save_folder + cfg['name']+ '_epoch_' + str(epoch) + '_noGrad_FT_WeightChanger.pth')
            epoch += 1
    ...
    torch.save(net.state_dict(), save_folder + cfg['name'] + '_Finally_FT_WeightChanger.pth')
